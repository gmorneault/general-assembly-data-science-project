...
numeric_cols <- sapply(loan.stats, is.numeric)
head(loan.stats[,numeric_cols==TRUE])

loan.stats.num <- melt(loan.stats[,numeric_cols], id="is_bad")
head(loan.stats.num)

ddply(loan.stats.num, .(variable), summarise, mean(value, na.rm = TRUE))
      
unique(loan.stats.num$variable)

p <- ggplot(loan.stats.num, aes(x=value, group=is_bad, colour=factor(is_bad)))
p + geom_density() + facet_wrap(~variable, scales="free")


qplot(loan_amnt, data=loan.stats, geom="density")
qplot(loan_amnt, data=loan.stats, geom="density", fill=loan_status)

qplot(int_rate, data=loan.stats, geom="density")
ggplot(loan.stats, aes(int_rate)) + geom_density()
qplot(grade, int_rate, data=loan.stats, geom="boxplot", fill=grade)

qplot(loan_amnt, data=loan.stats, geom="histogram", fill=grade, binwidth=2000)
qplot(grade, loan_amnt, data=loan.stats, geom="boxplot", fill=grade)



g <- ggplot(data = loan.stats, aes(x = loan_status, y = int_rate))
g <- g + geom_boxplot() + theme_bw()
g <- g + theme(axis.text.x = element_text(angle = 90, hjust = 1))
print(g)


plot(int_rate ~ loan_status,
     col = grade,
     data = loan.stats,
     xlab = "Loan Status",
     ylab = "Interest Rate",
     ...
     frame.plot='false',
     family="serif")
legend("topright", legend = unique(loan.stats$grade), col = 1:3, pch = 19)
